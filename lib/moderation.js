...
/**
 * Gibberish Detector
 * Matches 5+ consecutive consonants (Cyrillic or English).
 * Excludes vowels: a,e,i,o,u,y,а,е,ё,и,о,у,ы,э,ю,я
 */
const GIBBERISH_REGEX = /[bcdfghjklmnpqrstvwxzбвгджзйклмнпрстфхцчшщ]{5,}/i;

export function checkContent(text) {
    if (!text) return { safe: true, flagged: [] };
    if (GIBBERISH_REGEX.test(text)) return { safe: false, flagged: ['gibberish'] };
    // ... existing logic ...
...
export function validateTitle(title) {
    ...
    // Gibberish Check (Consonant Clusters)
    if (GIBBERISH_REGEX.test(trimmed)) {
        return { valid: false, errorKey: "validation_title_gibberish" }; // New key
    }
