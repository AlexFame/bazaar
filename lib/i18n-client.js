...
export function LanguageProvider({ children }) {
  ...
  useEffect(() => {
    ...
    // 1. Проверяем localStorage (ручной выбор пользователя - высший приоритет)
    ...
    // 2. Если нет сохраненного, пытаемся взять из Telegram WebApp
    ...
    if (tgLangDetected && SUPPORTED_LANGS.includes(tgLangDetected)) {
       ...
       window.localStorage.setItem(LANG_KEY, tgLangDetected); // Сохраняем как дефолт
       return;
    }
