...
export default function CreateListingClient({ onCreated, editId }) {
  ...
  const notificationOccurred = (...args) => {
    try {
      haptic?.notificationOccurred?.(...args);
    } catch (e) {
      console.warn("Haptic error:", e);
    }
  };
  const impactOccurred = (...args) => {
    try {
      haptic?.impactOccurred?.(...args);
    } catch (e) {
      console.warn("Haptic error:", e);
    }
  };
  ...
  useEffect(() => {
    ...
    const saveToLocalStorage = (data) => {
      if (
        !loading &&
        (data.title || data.description || data.price || data.listingUuid)
      ) {
  ...
  const handleBack = async () => {
    if (!editId && (title || description || price)) {
      ...
      if (success) {
        ...
        if (
          window.confirm(
            t("draft_save_error_exit") ||
              "Не удалось сохранить черновик в облако. Он останется на этом устройстве. Выйти?",
          )
        ) {
          router.back();
        } else {
      ...
      try {
        ...
        if (parsed.title || parsed.description) {
          ...
          if (
            dismissedAt &&
            parsed.timestamp &&
            Number(dismissedAt) >= parsed.timestamp
          ) {
            return;
          }
          ...
          if (!title && !description) {
            const message =
              t("restore_draft") ||
              "Найдено незавершенное объявление. Восстановить?";
            if (window.confirm(message)) {
              ...
              toast.success(
                t("draft_auto_restored") || "Черновик восстановлен",
              );
            } else {
  ...
  async function handleSubmit(e, status = "active") {
    ...
    console.log(
      "handleSubmit called with status:",
      status,
      "loading:",
      loading,
    );
    ...
    if (!accountAgeCheck.allowed) {
      ...
      toast.error(
        accountAgeCheck.reason ||
          t("tg_account_too_new") ||
          "Ваш аккаунт Telegram слишком новый.",
      );
      return;
    }
    ...
    if (!title.trim())
      errors.title = t("validation_title_empty") || "Заполните заголовок";
    if (!price.trim())
      errors.price = t("validation_price_invalid") || "Укажите цену";
    if (!description.trim())
      errors.description = t("validation_desc_empty") || "Добавьте описание";
    ...
    if (!contentCheckTitle.safe) {
      console.warn(
        "Blocked by title content check:",
        contentCheckTitle.flagged,
      );
      const errMsg =
        t("alert_forbidden_content") ||
        "Заголовок содержит запрещённый контент";
    ...
    if (!contentCheckDesc.safe) {
      console.warn(
        "Description content check flagged (non-blocking):",
        contentCheckDesc.flagged,
      );
    }
    ...
    if (hasEmoji(title)) {
      ...
      const errMsg =
        t("alert_no_emoji") || "В заголовке нельзя использовать эмодзи";
      setFieldErrors({ title: errMsg });
      toast.error(errMsg);
      titleRef.current?.scrollIntoView({ behavior: "smooth", block: "center" });
      return;
    ...
    try {
      ...
      for (const [index, img] of images.entries()) {
        ...
        let fileExt = img.file.name.split(".").pop().toLowerCase();
        let mimeType = img.file.type;

        if (mimeType && mimeType.startsWith("video/")) {
          if (!["mp4", "mov", "webm", "mkv", "3gp", "m4v"].includes(fileExt)) {
      ...
      if (
        ...
        const uploadSingle = async (imgObj) => {
          if (imgObj.file) {
            let fileExt = imgObj.file.name.split(".").pop().toLowerCase();
            let mimeType = imgObj.file.type;

            if (mimeType && mimeType.startsWith("video/")) {
              if (
                !["mp4", "mov", "webm", "mkv", "3gp", "m4v"].includes(fileExt)
              ) {
                fileExt = mimeType.split("/")[1] || "mp4";
                if (fileExt === "quicktime") fileExt = "mov";
              }
            ...
            const { data, error } = await supabase.storage
              .from("listing-images")
              .upload(
                `listing-${editId || listingId}/${uniqueFileName}`,
                imgObj.file,
                {
                  contentType: mimeType,
                  upsert: true,
      ...
      if (status === "draft") {
        ...
        if (editId) {
          ...
          toast.success(
            t("listing_edited") || "Объявление отредактировано! ✅",
            {
              description:
                t("returning_to_listing") || "Возвращаемся к объявлению...",
            },
          );
          notificationOccurred("success");
  ...
  const renderDynamicField = (filter) => {
    ...
    if (
      listingType === "service" &&
      (filter.key === "material" || filter.key === "manufacturer")
    ) {
      return null;
    }
    ...
    if (
      ...
      (filter.key === "memory" || filter.key === "ram")
    ) {
    ...
    if (
      (categoryKey === "kids" || categoryKey === "fashion") &&
      (filter.key === "size_clothes" || filter.key === "size_shoes")
    ) {
      const currentSubtype = parameters["subtype"];
      // If clothing, hide shoes size. If shoes, hide clothing size.
      // If neither (e.g. accessories), hide both.
      if (filter.key === "size_clothes" && currentSubtype !== "clothing")
        return null;
      if (filter.key === "size_shoes" && currentSubtype !== "shoes")
        return null;
    }
    ...
    if (categoryKey === "kids") {
      ...
      if (
        ["toys", "food", "books"].includes(currentSubtype) &&
        (filter.key === "brand" || filter.key === "material")
      ) {
        return null;
      }
    ...
    if (
      categoryKey === "realty" &&
      (filter.key === "deposit" || filter.key === "utilities_included")
    ) {
        ...
        <section className="w-full max-w-xl mx-auto mt-4 px-3">
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-2xl px-3 py-3 text-xs text-black/80">
            <div className="font-semibold mb-1">
              {t("open_via_telegram_title") || "Открой через Telegram"}
            </div>
            <p className="leading-snug">
              {t("open_via_telegram_desc") ||
                "Создавать объявления можно только если ты открыл Bazaar из Telegram-бота. Открой бота, нажми кнопку с WebApp и попробуй ещё раз."}
            </p>
          </div>
        </section>
      );
    ...
    <div className="min-h-screen bg-white dark:bg-black pb-24">
      <div className="sticky top-0 z-10 bg-white/80 dark:bg-black/80 backdrop-blur-md px-4 py-4 border-b border-gray-100 dark:border-white/10">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold mb-4 dark:text-white text-center">
            {t(editId ? "create_title_edit" : "create_title")}
          </h1>
        </div>
      </div>
      ...
      <div className="w-full max-w-xl mx-auto mt-4 px-3">
        ...
        <form
          ...
          noValidate
          className="bg-[#F2F3F7] dark:bg-white/5 rounded-2xl p-3 border border-transparent dark:border-white/10"
        >
                    ...
                    {categoryKey === "realty" ? (
                      <>
                        ...
                        <option value="new_building">
                          {t("condition_new_building")}
                        </option>
                        <option value="secondary">
                          {t("condition_secondary")}
                        </option>
                      </>
                    ) : (
                      <>
